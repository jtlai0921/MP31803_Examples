...
namespace msgbox1
{
    public partial class Form1 : Form
    {
        ...
        int num;                // 宣告為整數變數，存放錯誤次數 所有事件一起共用

        private void btnLogin_Click(object sender, EventArgs e)
        {
            DialogResult result;
            num += 1;        //次數加1
            if (txtID.Text == "google" && txtPW.Text == "1688")        // 判斷帳密是否正確
            {
            ...
            else
            {
                ...
                {
                    result = MessageBox.Show("你的帳號密碼有誤，剩下" + (3 - num) +
                  "次！ 是否重新輸入？", "帳號密碼錯誤", MessageBoxButtons.YesNo);
