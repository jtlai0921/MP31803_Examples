    ...
    class Program
    {
        static void Main(string[] args)
        {
            try   //監控可能會發生例外的程式碼
            {
                ...
                Employee[] per = new Employee[n];        //產生per[0]~per[n-1] 的陣列元素
                for (int i = 0; i <= per.GetUpperBound(0); i++)
                {
                    per[i] = new Employee();     			        //建立per[i] 物件的實體
                    Console.WriteLine("請輸入第 " + (i + 1).ToString() + " 筆員工記錄");
                    Console.Write(" 編號：");
                    per[i].EmpID = Console.ReadLine(); 	    //輸入編號
                    Console.Write(" 姓名：");
                    per[i].EmpName = Console.ReadLine();  	//輸入姓名
                    Console.Write(" 薪資：");
                    per[i].EmpSalary = int.Parse(Console.ReadLine());   //輸入數量轉型成整數
                    Console.WriteLine("---------------------------");
                    Console.WriteLine("顯示第 " + (i + 1).ToString() + " 筆員工記錄");
                    per[i].ShowInfo();
                }
